...
class Viewer(object):
    ...
    def render(self, env, return_rgb_array=False):
        ...
        self.window.set_caption(f"Step: {env.current_step}")
    ...
    def _draw_players(self, env):
        ...
        for p in env.players:
            # if p.reward != 0:
            #     self._draw_reward_badge(*p.position, p.reward, player=True)
            self._draw_badge(*p.position, p.player_id, player=True)
            self._draw_last_action_badge(*p.position, p.last_action, player=True)

    def _draw_badge(self, row, col, level, player=False):
        resolution = 6
        ...
        label = pyglet.text.Label(
            ...
            anchor_x="center",
            anchor_y="center",
            color=(*_BLACK, 255) if not player else (*_WHITE, 255),
        )
        label.draw()

    def _draw_last_action_badge(self, row, col, last_action, player=False):
        resolution = 6
        radius = self.grid_size / 5
        ...
        circle.draw(GL_LINE_LOOP)
        label = pyglet.text.Label(
            str(actions_dict[last_action]) if last_action is not None else " ",
            font_name="Times New Roman",
            font_size=12,
            bold=True,
            x=badge_x,
    ...
    def _draw_reward_badge(self, row, col, reward, player=False):
        resolution = 6
        radius = self.grid_size / 5

        badge_x = col * (self.grid_size + 1) + (2 / 4) * (self.grid_size + 1)
        badge_y = self.height - (self.grid_size + 1) * (row + 1) + (3 / 4) * (self.grid_size + 1)
